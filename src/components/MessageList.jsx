        ...
        return (
          <React.Fragment>
              <Form onSubmit={ () => this.props.postMessageText( this.state.message ) }>
                ...
                <ul>
                  { this.props.messageList.map( message => <Message key={ Date.now().toString() + message.id } username={ message.id /* this.getPostUsername( this.props.userList, message.id ) */ } message={ message.text } numLikes={ message.likes.length } /> ) }
                </ul>
            </React.Fragment>
        );
    }
